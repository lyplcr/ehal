...
{
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DIR) &=~pins;
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DEN) |= pins;
}
...
static inline void gpio5_set_dir(uint8_t pins, uint8_t in)
{
	gpio5_set_in(pins  &  in);
	gpio5_set_out(pins &(~in));
}

static inline void gpio5_set(uint8_t pins)
{
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DATA + (pins << 2)) = pins;
}

static inline void gpio5_clr(uint8_t pins)
{
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DATA + (pins << 2)) = 0;
}

static inline void gpio5_set_to(uint8_t pins, uint8_t high)
{
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DATA + (pins << 2)) = high;
}

static inline uint8_t gpio5_read(void)
{
	return HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DATA);
}

static inline void gpio5_set_pullup(uint8_t pins)
{
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_PUR)  |= pins;
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_DR2R) |= pins;
}
...
{
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_IS)  &=~(rise | fall); /* on edge */
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_IBE) |= (rise & fall); /* both edges */
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_IEV) &=~(fall); /* mark single edge */
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_IEV) |= (rise); /* same here */
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_ICR) |= (rise | fall); /* clear IRQs */
	HWREG(GPIO_PORTF_AHB_BASE + GPIO_O_IM)  |= (rise | fall); /* will IRQ */
}
